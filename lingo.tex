...
inherited features. \PFS{Several measures that quantify the of
...
analysis used only the core concepts \PFS{included in word lists
  ...
  in linguistic history \cite{Lee:11,Gray:09,Dunn:11,Gell-Mann:11}}, and
...
compared against expert linguistic judgements. \PFS{It remains an open
  question at this point how well computational approaches to cognate
  identification will generalize to deep divergence times; the success of
  computational biology in the analogous task of detecting remote
  homologies \cite{Noble:05,Haggerty:14,Chen:16} certainly is encouraging.}
...
\PFS{PLEASE CHECK THE LIGHT GREEN TEXT ABOVE AND BELOW!} 
...
correspondence is higher in this part of the vocabulary.  \PFS{Finally,
  cognacy is defined as a binary relations between words. When word pairs
  ...
  may be missed. To deal with such situations multiple alignments may be
  scored directly, either using the same probabilistic model that generated
  the initial cognate calls, or with the help of consistency-based scoring
...
\TODO{TANMOY: is the a better place for this comment? I couldn't really fit
  it into text elsewhere. Maybe add a sentence or just a reference how this
  is handled in Hruschka 2015?}

...
known in the biological phylogenetic literature. \TB{This graded
  ...
  general feature of what statistical analysis can be used to quantify
  in order to avoid the loss of predictive capability in
  an attempt to fit complex models with limited data. In this case,
  the value of this approach can be explained as follows: since typically
  the vertical inheritance arises from starkly different processes
  like population migration and fission than contact phenomena that
  lead to horizontal influences, building trees rather than
  reticulated networks allows us to
  answer the former questions even when the data are not sufficient to
  parameterize the entire process.} \TODO{The logic here is
...
corresponding to the different threshold values... TANMOY 
...
incertitude expressed in the above passage. TANMOY}
...
proto-languages and then the regularities of sound change.  \TODO{This is
...
tasks is merely a matter of computational efficiency and expediency. In
...
computed for a given tree, and then used to compare trees. Experience from
computational biology, furthermore, shows that it is often pragmatically
favorable to perform as much of the analysis as possible on directly
observable data. \NEW{For example, methods that attempt to concurrently
  estimate alignments and phylogenetic trees \cite{Lunter:05,Heman:14} have
  been devised and applied with success, it appears that the gain in
  accuracy over the common practise (of treating alignment and phylogeny
  reconstructing as independent tasks) is very moderate and often does not
...
\TODO{Is this a language independent result? DAMIAN TANMOY}
The signal

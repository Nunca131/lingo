...
Franz Steiner Verlag: Stuttgart. 111-124.}\NR{latter is unsuited since we try 
to give a short overview on the history? Apparently, the first still holds and 
this person's French is not that good?}
...
An important issue to consider in the choice of traits to analyze is
  ...
  sound-meaning correspondences encoded as lexical roots 
  \TODO{On page 2, lines 26-29 it is argued that 'individual sound-meaning 
  correspondences encoded as lexical roots' might be particularly prone
to homoplasy. Insofar as sound-meaning correspondences are arbitrary (i.e.
excluding things like iconicity), we might expect relatively low
rates of homoplasy in these items. Typological features of language 
might be a clearer example to use to illustrate the notion of homoplasy in 
linguistic traits.}
  might be more
...
branches \NEW{by means} of data for related species \cite{Rouse:16}. This
...
\TODO{The discussion of Swadesh lists ignores their shortcomings, specifically 
non-universal nature of some meanings (e.g. dog, generic 
tree), semantic relationships that may influence forms of multiple items 
(e.g. moon/night, red/blood), noun-heaviness, etc.A reference to 
an article discussing some of this would provide some balance. One 
article that illustrates many of these issues is Hoijer (1956) 
Lexicostatistics: A critique. Language 32(1): 49-60. I suspect there 
are more recent pieces that would also suffice.}
...
may be denoted by superposed apostrophe \NEW{$<$}\'k\NEW{$>$}, postposed apostrophe
\NEW{$<$}k'\NEW{$>$}, postposed palochka \NEW{$<$}k$|\NEW{>}$, or special letters
\NEW{$<$}\cmr\texthtk\NEW{$>$}. Depending on the encoding, these might be represented either
...
\TODO{Leaving the sequences of characters to be handled later leaves 
the reader wondering, at this point in the paper, how this will
be resolved. A reference to where it is resolved would be useful.}
...
e.g.][]{Notredame:07}, it is the construction of good \TT{scoring models} that
...
vowels in the pre-alignments 
\TODO{[...] "pre-alignment", which appears
by p. 9, but is introduced with small caps and sketchy definition only on
p. 12)}
is a good working assumption, there is at
...
together with the contributions of all gaps\NEW{,\ie{} insertions or 
deletions of characters}, and then a normalized score
...
sound/symbol in language A corresponds to two in language B
(e.g. Spanish puerta - Italian porta, or instances where two symbols 
correspond to a single sound in one language). This is one of the reasons 
that I recommend consolidating the discussion of representational 
issues and being more explicit about what "gaps" are in this method.}
\NR{For this todo maybe point to Fig.~\ref{fig:aln}? maybe I can find 
another alignment where there is no deletion score used even though there 
obviously is one.}
...
\caption{Aligning words using a likelihood based method can be used to
  ...
  the entire dataset. 
  \TODO{ I don't think the last sentence ("Note that although only a 
  small detail...") is necessary given the descriptions
...
\TODO{do you mean the "alternative"}
Nevertheless,
...
\TODO{At least cite Holman and Wichmann's paper in Language for the discussion
...
\TODO{Do you mean "typographical errors"? As written, the meaning of this sentence is unclear.}
...
to the effect under consideration \NEW{(see~\cite{Oswalt:70} for an example
...
Colin Renfrew, April McMahon and Robert Lawrence Trask (eds.), Time

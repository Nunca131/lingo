...
\NEW{incomplete} lineage sorting \NEW{\footnote{Incomplete lineage sorting
    ...
    contradicts the true phylogeny by \TB{an ancestral form being retained 
...
relationships whenever there exists a dominating set of \TB{co-inherited}
features. \PFS{Several measures that quantify the of
...
compared against expert linguistic judgements. \PFS{\TB{If 
  ...
  homologies \cite{Noble:05,Haggerty:14,Chen:16} \TB{is certainly} encouraging.}
...
\PFS{PLEASE CHECK THE LIGHT GREEN TEXT ABOVE AND BELOW! DONE} 
...
correspondence is higher in this part of the vocabulary.  \PFS{Finally,
  cognacy \TB{computed in this fashion is} a binary relations between words. When word pairs
  ...
  may \TB{not be so classified}. To deal with such situations multiple alignments may be
  scored directly, either \TB{using the same probabilistic model that generated
...
\TODO{TANMOY: is the a better place for this comment? I couldn't really fit
  ...
  is handled in Hruschka 2015? DONE}

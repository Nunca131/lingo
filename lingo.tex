...
languages~\cite{dUrville:1832}. These early roots predate Darwin's first
...
may not follow the same lines as the core bundle.
...
phylogeny of any of these sets of features. \TB{When we are confident
...
\TB{An important issue to consider in the choice of traits to analyze is
  ...
  classification.  On the other hand, less complex items such as individual
  sound-meaning correspondences encoded as lexical roots might be more
...
reconstruction are often the method of choice~\cite{Waegele:DMP}. Of course
no method is infallible. If features are chosen that are either too noisy
to convey retrievable information or that are not strictly vertically
...
strange case of \emph{Xenoturbella}. This unimpressive worm-like animal was
...
\TT{demoted} again earlier this year to one of the most basal bilaterian
branches with the help of data for related species \cite{Rouse:16}. This
underlines the fundamental importance of basing phylogenetic reconstruction
on multiple, independent feature sets.

Just like language evolution, biological evolution is neither purely
vertical nor completely tree-like. Even in higher organisms, horizontal
gene transfer, convergent adaptive evolution, lineage sorting, and random
noise in the data introduce deviations from tree-likeness in the
data. Lineages are not always well-separated, as in the case of \TT{ring
  species} \cite{irwin:2005} in biology and \TT{dialect continua}
\cite{heggarty:2010,franccois2015trees}, making the tree model only an
approximation.  Although horizontal transfer, like any other causes of
incorrect tree estimation, may bias quantities such as divergence time
estimated from trees~\cite{Greenhill:09}, there are nevertheless many
...
the presence of other forms of inheritance, trees are a useful
representation of evolutionary relationships whenever there exists a
dominating set of vertically inherited features.
...
inheritance \cite{Bowern:10}. As in other loosely constrained statistical
...
goodness of fit, \eg{} through low likelihoods in particular branches or a
wide spread in the posterior distributions in Bayesian settings. The key
...
lists---assumed to be the most closely co-inherited roots, refractory to
horizontal transfer---or, at least, weighted their evidence more, this gave
a very good fit with known linguistic
history~\cite{Lee:11,Gray:09,Dunn:11}, and provided a credible starting
...
automatic method to determine cognacy. Any progress in the development of
such methods has to rely on a good inference of sound correspondences,
which seems to require larger datasets.

Beyond lexical data, there have been several attempts to use other kinds of
...
between word forms within a cognate set~\cite{Heggarty:00}. In addition,
machine learning approaches have been used in reconstructing ancient word
forms~\cite{BouchardCote:13} and in phylogenetic analysis of typological
features~\cite{Gell-Mann:11,Dunn:11}. One must note, however, that even
when there appears to be a phylogenetic signal in such data, there is much
less consensus about the nature of any vertically-inherited \TT{core} and
how that relates to the vertical inherited bundle of features traditionally
used to define genetic relations in linguistics~\cite{Dediu:13}. In using
such data, care must, therefore, be exercised.
...
approaches. Small or moderate size word lists contain only very limited
numbers of cognates, particularly for the more distant languages. The
recognition of cognates between distant languages often requires the
...
becomes a central issue for large-scale language phylogenetics.
...
The core or basic vocabulary comprising the Swadesh concepts when compiled
...
method \cite{Pagel:2013}. Lexical replacement rate seems to be linked to
...
(/\cmr\texthtk/). Depending on the encoding, these might be represented either
as digraphs or unigraphs. If the language specific segmentation is
...
characters to be handled later.
...
e.g.][]{Notredame:07}, it is the construction of good scoring models that
...
over evolutionary time-scale. In languages, on the other hand, the phonetic
...
\caption{A hypothetical language tree for 5 (proto-)languages (dark blue)
  and one (proto-)sound (pink). Greek letter correspond to reconstructed
  ...
  words where language $B$ has sound $b$. It thus indirectly imply that $a$
...
consonants than vowels. Another example is due to \citet{Heggarty:00}, who
...
suffices to identify at least part of the cognate pairs in the data
...
the the correct alignments of real cognate pairs for the statistical
...
vowels in the pre-alignments is a good working assumption, there is at
...
pre-alignments that are classified as candidate cognates. Hence in the
...
compare it a simpler model with unigram scores. The idea behind the
computation of the scoring function is the same for both model. Likely
...
of independent evolution \cite{Durbin:98,Altschul:10}.

\begin{figure}
\begin{center}
\includegraphics[width=0.5\textwidth]{figures/pilaga_mocovi_bigramscoring}
\end{center}
\caption{Example alignment for words from Mocov{\'\i} and Pilag{\'a}
  representing the concept \QUOTE{world}. Each bigram pair in the alignment
  is scored separately, resulting in a total score $\sigma=17.06$ in this
  example. Thus the normalized score is
  $\textit{nscore}=17.06/(8-2)=2.84$.}
\label{fig:aln}
\end{figure}
...
Based on this, one can proceed to define the total score as the sum of all
bigram scores $\sigma = \sum_{\alpha, \beta} \sigma_{AB}(\alpha,\beta)$
together with the contributions of all gaps, and then a normalized score
...
without \textasciicircum{} and \$. Such a normalization makes alignments of
words with different lengths comparable by reporting an average score per
aligned position.
...
alignments of words from 11 Indo-European languages. For both scoring
...
sound along with word boundaries, an imperfect proxy for syllable
boundary. In more difficult cases, such as the Guaycuruan example, the
situation is far from perfect. Here the cognates are not completely
separated from the non-cognates by the \textit{nscore} even for bigram
...
\ie{} we are left with a trade-off between between false positives (false
...
of the threshold is left to us. Depending on the analysis where such
...
represents the weight of phonetic evidence in favor of cognacy. Any further
deductions obtained from such putative cognates can weight the evidence by
this score, thereby lowering the deleterious effect from false positives.
...
step from a crude \TT{pre-alignment} to detailed scoring model yields a
...
features. The definition of scores described above generalized in a
...
between the observable data as directly as possibly, without making \emph{a
  priori} assumptions on how exactly languages evolve. An alternative
...
smaller than the cutoff. Each cutoff values thus yields a pair of
sensitivity and specificity values that quantify how well the score-based
separation between cognates and non-cognates performs. It is customary to
...
quality of the scoring system. In the Fig.~\ref{fig:DanROC} the AUC is
$0.9$.
...
\caption{Aligning words using a likelihood based method can be used to
  ...
  non-cognates (Area-under-the-curve AUC$=0.90$). Note that although only a
  small detail of the entire data set, namely the first four meanings
  sorted by their English description, is shown on the left, the ROC uses
  the entire dataset.  }
\label{fig:DanROC}
\end{figure}
...
the expectation that the enrichment of cognates and the regularity of
...
focus on alignments and their score conforms more closely to the paradigm
...
closer to the traditional conceptualization in linguistics.  Nevertheless,
...
probabilities \cite{Pennisi:05,Liberles:07}. At present there is no method
to infer sound changes rules directly from a scoring system. Mathematically
...
feasible at least from a computational point of view, it may not be the
...
identification and the identification of regular sound changes. One may
...
regularities of sound change. This separation of tasks is merely a matter
...
\TB{Another difference between the phylogenetic problem in computational
...
Similarity serves only a first filter for relatedness. In fact, the rate of
\QUOTE{random} mutations variously affecting the sounds in individual words
...
difference between two language lineages form consistent systems applying
...
signal. It should be noticed that the methods discussed before will yield
...
Let us flesh out with more detail a concrete example. In the case of
...
to the effect under consideration.
...
literature, we elaborate on the example in Fig.~\ref{fig:permut}. The data
...
parametric method but do not need to specified in the non-parametric case.
...
should weight the stable words and consonantal identity more. The signal
...
producing the correct results even though some aspects or parameter of the
...
a notion of uniformatarianism: that the process of language change and
diversification has not changed much over time. In reasonably homogeneous
environments and for short time durations, this is a reasonable assumption;
and in very heterogeneous, rapidly changing, or extremely long time
durations the changes in process can be averaged over and approximated by
...
problem. Non-stationarity in this cases usually means that the probability
...
the final confidence interval.\footnote{The confidence interval describe
...
their material culture. Little evidence about the cultural organization of
the Tasmanian people has survived, with different accounts differing in the
degree of unity or diversity across the island. To worsen the situation,
...
needs a model about the rates of change; equally, however, the known dates
...
severely criticized since early on~\cite{Bergsland:62}, although some
support for the idea still exists in other
contexts~\cite{Ehret:00,Starostin:00,Starostin:13,Holman:11}. Within the
...
phenomenon?  In other words: do data source accessible to us contain
...
from paralogous cognates: \ie{} processes like sound symbolism are
...
they will appear at random places in word list. On the other hand,
...
\textsc{all} language have a $\delta$-value larger than zero.
...
A recent work~\cite{Youn:16} provides a quantitative assessment on the
...
by a polysemy. The figure \ref{fig:semanticnet} shows an aggregated network
...
languages were significantly correlated with that obtained from the
...
We described in the previous sections several statistical method and
discussed their application of different aspects of language evolution.  We
demonstrated in sample application that the results are reasonable.  One
...
Statistics provides measure for the confidence in the results produced by a
computational method. Throughout this constribution we have used three of
...
are encoded in a linear manner, and their comparision is usually based on
...
\PFS{the uncritical use of} molecular phylogenetics programs directly for
...
\PFS{In our presentation above we have highlighted but a few aspects of
  ...
  work of \citet{Dunn:11} highlights an important issue: on the one hand,
...
true or not but rather how likely they are. Thus, when data is noisy,
...
\textsc{what questions are interesting and what constitutes adequate
...
Word alignment are performed with an extension of Needleman-Wunsch
...
penalize longer gaps subadditively. The simplest such model are affine gap
...
accommodate the systematic silencing a phone. Here we use language and
...
all scores remain finite. For the scoring model employed here we used
$\varepsilon=3$. 

...
wide spread in the posterior distributions in Bayesian settings or explicit 
measures of reticulation when network approaches are applied to the same data \cite{Robinson:2012}. 
...
estimated (branch lengths, rates of change etc). PETER} 
\DB{I disagree - the text is already too gigantic! Added some references where people 
can see some (suboptimal) strategies used in linguistics, e.g. compute reticulation 
in the same data. Wide posteriors are taken as abscence of evidence for the 
relation but rarely used to discuss against the tree-model.} The key
...
ARGs to begin with? (That would be systematic, rather than ad hoc). TANMOY} 
\DB{I feel the whole discussion on the tree/non-tree like tension is too long already...}
...
\TODO{The text and formulas here are ambiguous as to whether sigma and
  ...
  and the domains of free variables. PETER check, wheter we keep Lydia's 
  changes and simplify long index in sum over all bigram scores}  Based on this, one can proceed
...
to the Needleman Wunsch algorithm still keeps it polynominal. PETER}
and
...
approach from the perspective of propagating uncertainty. TANMOY}
...
the text stands this claim is completelty unsupported. PETER}
...
\TODO{The final sentence of this paragraph needs to be reworded for clarity. PETER}

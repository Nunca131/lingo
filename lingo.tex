...
\begin{abstract}
  ...
  then be systematically refined\NEW{, always keeping} explicit the
  ...
  assumptions. \NEW{These methods, though, often appear as a `black box' to
  ...
  shift, attributed to the advent of big\NEW{, but often incomplete and
    poorly curated,} data, \NEW{emphasizing the underlying similarity of
    the newer quantitative to the traditional comparative methods and
    discussing when and to what extent the former have advantages over the
    latter. In this review, we cover briefly both} randomization tests for
  detecting patterns in a largely model-independent fashion and
  phylolinguistic methods for a more model-based analysis of these
  patterns. We foresee a fruitful division of labor between the ability to
  computationally process large volumes of data and the trained linguistic
  insight declaring worthy prior commitments and interesting hypotheses in
  need of comparison.
...
vertically inherited features range from \NEW{phonology} to individual
lexical items and entire grammatical structures. Just like the genotype
comprising a biological system has a core subset of housekeeping genes that
form a bundle that is frequently, but not always, co-inherited, it is
likely that language has a core that can be used to define a \TT{vertical
  descent}.  Many other traits would be more or less coherent with respect
to this bundle of features, with deviations defining \TT{horizontal}
...
\NEW{It is an entirely empirical question which features belongs to which
  ...
  core components of indispensable subsystems can be transmitted
  ...
  of any particular core gene, see \citet{Betat:15a} for a case study.}  In
the following, we \NEW{therefore} do not consider questions that pertain to
...
sets of features. When we are confident\NEW{---for the analysis of the
...
strange case of \emph{Xenoturbella}, \NEW{which was placed at vastly
  ...
  sets and taxa included for comparison \cite{Bourlat:06,Rouse:16}.}  This
...
\NEW{In particular, low likelihoods in particular branches and a wide
  ...
  not only by insufficient signal but also by reticulated patterns in the
...
languages from the largest linguistic families in the world
...
alignment, \NEW{which we call \TT{pre-alignments}, that is then used to
  extract more detailed information about sound correspondences.}  The
\NEW{initial rules} can be very simple. \citet{Steiner:11a}, for example,
...
Based on this, one can proceed to define the total score \NEW{$\sigma(u,v)$
  ...
  it is preferable to used a} normalized \NEW{similarity} score
...
to cognates.\footnote{\NEW{We note that the unrelated words are slightly
    more \emph{dissimilar} than one would expect random pairs to be.  We do
    not study this further here, but it would be expected if, on average,
...
in Indo-European, but the two peak structure is still visible. \NEW{The
  difference is not necessarily significant---the exact distributions
  depend strongly on the amount of training data available.}
...
compared against expert linguistic judgments. \NEW{If 
  ...
  to detect cognates in such saturation's as well.  Whether this really
...
feasible at least from a computational point of view, \NEW{as exemplified
...
should weight the stable words and \NEW{sounds (e.g., word-initial nasals) as these
are truly robust across many families.} The signal from vowels is more informative when
...
a notion of \NEW{Unitarianism}: that the process of language change and
...
traditional historical linguists, \NEW{their real value is in situations
  where the evidence is scarce or contradictory.  In such cases, the
  results of the analysis depend on prior beliefs and assumptions about
  consistency with global patterns.  Whereas a trained linguist can see
  that the evidence is weak, the quantitative method is often better able
  to integrate such weak evidence from various pieces of data, keeping
  track of the accumulated weight of evidence. Thus, for example, while the
  most widely accepted opinion is that} linguistic analysis can lead at
most to rejecting the idea that Tasmanian languages constitute a single
linguistic family \cite{Crowley:1981}, Bowern \NEW{\cite{Bowern:12a}} was
able to \NEW{use quantitative methods to} gather enough evidence for
4\NEW{--}5 groupings that correspond somewhat with earlier archaeological
classifications, while at the same time lending support to the idea that
Tasmanian languages are not all directly related \NEW{at} the time depth
accessible to \NEW{available} methods and the data.
...
ancestor. \NEW{Paralogy is a concept different from polysemy: paralogous
  words are by definition distinct, and thus phonologically distinguished,
...
explainable by chance, can result only from paralogous cognates, \ie{}
...
\NEW{Nakh-Daghestanian} languages \NEW{(mostly form the Daghestanian
  branch, Nakh languages marked with asterix)}, as well as for a sample of
155 languages from the IDS \NEW{(see Table~\ref{tab:langDeltas})}. If
similar, non-identical words do not systematically have similar meanings we
expect that this distribution is centered around zero.

\begin{figure}
\begin{center}
\includegraphics[width=0.45\textwidth,clip=]{figures/deltaplot}
\end{center}
\caption{Distribution of $\delta$ in Indo-European, \NEW{ Nakh-Daghestanian}, and
...
In our presentation above we have highlighted but a few aspects of
  ...
  physical, or biological constraints from those that are a mere reflection
  of lineage-specific persistence. As a case in point, some of us recently
  revisited \cite{Blasi:16a} the presumed independence of sound and meaning,
  ...
  a mechanistic explanation for most of the statistically significant

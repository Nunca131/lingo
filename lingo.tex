...
imperfect proxy for syllable \NEW{boundaries} \DB{(namely, those recoverable 
...
for each choice of the cutoff. \DB{Finally, these scores can be 
...
time depth \DB{(so that cognacy is likely to be reflected in the surface 
...
probabilistic models focuses on the evolutionary processes\DB{. Nevertheless, 
...
computational techniques \DB{and different prior information or expert knowledge}.
...
of computational methodology. For example, \DB{hypotheses about the 
...
at an early stage. Thus, for example, \DB{errors in the coding of linguistic data} often introduce
...
many \DB{remove: non-alphabetic} writing systems do not directly encode the phonetics at
all.
...
cognate pairs. \DB{Naturally, these claims are subject to regularities that 
...
molecular clock hypothesis in molecular evolution. Rate \DB{regularity} has been
...
context of a likelihood based approach, the \DB{regularity} of the rates

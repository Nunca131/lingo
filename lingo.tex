...
in these networks? see text} \DB{yes for non-vertical transmission}
...
estimated (branch lengths, rates of change etc). } \DB{I disagree - the text is already too gigantic! Added some references where people can see some (suboptimal) strategies used in linguistics, e.g. compute reticulation in the same data. Wide posteriors are taken as abscence of evidence for the relation but rarely used to discuss against the tree-model.}
The key
...
be resolved. A reference to where it is resolved would be useful.} \DB{I would change the last two sentences for a final reference in the previous sentence: "before analysis (although chances are such information is missing from composite data, which requires different strategies, ref. CHAPTER)}

...
no method is infallible \NEW{with \TB{respect} to the features chosen}. 
If \NEW{these} are either too noisy
to convey retrievable information or are not \TB{transmitted along the same tree}
to begin with the \TB{reconstructed} tree might not depict \TB{any} actual
...
\PFS{because it is often not obvious \textit{a priori} which features are
  informative\TB{ DELETE because it is introducing a new concept:
 \begin{tiny}and which taxa are relevant for comparison\end{tiny}} in a given context.}
...
\TODO{The reader may be left wondering about situations where one
  ...
  being more explicit about what "gaps" are in this method. NANCY}  \NR{For this
...
common ancestor. \NR{Both traits can undergo language specific changes 
resulting in differences in at least their pronology, i.e. this is the 
defining trait for distinction since we do not want to capture polysemy
of words.}
Conversely, we assume that a simultaneous similarity \NR{(but not equality)} of
...
plural). NANCY \NR{see senctences before. Should be clear now?}}
The
...
\NR{Conclusion and Additional Remarks}

...
\PFS{In particular, low likelihoods in particular branches and a wide
...
\textbf{Discussion of comment: delete when resolved!}
\par
\TODO{This is an important point which, unfortunately, is not developed in
  the paper. It would be great to be told how measuring the goodness of fit
  of various tree-models tells us whether a non-tree model fits better or
  worse. Also, the suggested paradigm of looking at the posterior in
  Bayesian settings conflates effects due to the desired quantity
  (tree-like-ness) and other things estimated (branch lengths, rates of
  change etc).} \DB{I disagree - the text is already too gigantic!  Added
  some references where people can see some (suboptimal) strategies used in
  linguistics, e.g. compute reticulation in the same data. Wide posteriors
  are taken as abscence of evidence for the relation but rarely used to
  discuss against the tree-model.  \PFS{says: any more references that we
    should add in the colored text above?}}
\end{minipage}}
...
known in the biological phylogenetic literature.  \TODO{The logic here is
...
\TODO{The text and formulas here are ambiguous as to whether sigma and
  nscore are defined of a pair of languages or a pair of words from two
  lgs.  Presumably the latter but actually the sigma formula as written
  contains free variables that ranged over the full set of pairs between
  two languages on the previous page. Formulas are used for clarity so
  please be explicit as to what is defined over what and the domains of
  free variables. PETER check, wheter we keep Lydia's changes and simplify
  long index in sum over all bigram scores} \PFS{check rephrased text and
  expressions below!}
...
\TODO{The reader may be left wondering about situations where one
  ...
  being more explicit about what "gaps" are in this method.}  \NR{For this
...
Based on this, one can proceed to define the total score \NEW{$\sigma(u,v)$
  ...
  in Fig.~\ref{fig:aln}. This ansatz makes it possible also to account for
...
\TODO{These [local transpositions] were assumed to be so few as to
  negligible earlier in the paper so that is not much of an
  improvement. But if it were important to capture them, k >= 3 is not
  necessary as adding transposition operations to the Needleman Wunsch
  algorithm still keeps it polynominal. PETER}
  \PFS{see text below} 
\end{minipage}}
...
features. \PFS{Transpositions, for instance, could be built explicitly into
...
depending on their frequency in the data. \PFS{One of the advantages of
...
from a computational point of view is needed... PETER}
...
observable data. \PFS{For example, methods that attempt to concurrently
...
directly. \PFS{The regular correspondences between all pairs of languages
...
\textbf{Discussion of comment: delete when resolved!}
 ...
 \TODO{Please
  explain what experience from computational biology shows how this is
  helpful if not for computational efficiency and expediency? As the text
  stands this claim is completelty unsupported. PETER}
...
transfer \cite{Dufraigne:05}. \PFS{The key idea there is that horizontally
...
artificially recoded set. We found that 97\% of the cognates \PFS{pairs
...
\par\framebox{\begin{minipage}{0.9\textwidth}\small
\textbf{Discussion of comment: delete when resolved!}
\TODO{The final sentence of this paragraph needs to be reworded for
  clarity. PETER}
\end{minipage}}
...
(emprirically) the idea of rate regularity. TANMOY}
Within the
...
one (that only paralogous cognates have sim in form and meaning). NANCY
def paralog and what is the process behind their generation plus same 
word for same meaning not counted as paralog}
...
plural). NANCY refer to the paralog definition before}
The
...
Otherwise: do you know who wrote this?} \NR{Ich kann es auch nicht finden.}
...
\section{Conclusion \PFS{and Future Directions}.} 

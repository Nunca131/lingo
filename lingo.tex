...
\LM{incomplete} lineage sorting \LM{\footnote{Incomplete lineage
    ...
    distantly related language.}}, and random noise in the data
introduce deviations from tree-likeness in the data.  \TODO{What is
  lineage sorting? \LM{I have inserted a footnote}} Lineages are not
always well-separated, as in the case of \TT{ring species}
\cite{irwin:2005} in biology and \TT{dialect continua}
\cite{heggarty:2010,franccois2015trees}, making the tree model only an
approximation \NEW{(see~\cite{Geisler:13} for further models for
  language relationships)}. Although horizontal transfer, like any
other causes of incorrect tree estimation, may bias quantities such as
divergence time estimated from trees~\cite{Greenhill:09}, there are
nevertheless many important events in the evolutionary history that
are correctly expressed by trees because they are primarily conserved
vertically. In fact, even in the presence of other forms of
\LM{relationships}, trees are a useful representation of evolutionary
relationships whenever there exists a dominating set of vertically
inherited features.  \TODO{This is a circular assessment (trees are
...
characters to be handled later. \LM{An example for how to solve this issue will be shown later in this section.} 

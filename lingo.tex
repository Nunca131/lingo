...
\cortext[cor]{Correspondence should be addressed to \TB{TB (\email{tanmoy@santafe.edu}) and 
...
\begin{abstract}
  ...
  data is revolutionizing many branches of linguistics.
% , while at the same   time uncovering new challenges.
  Overall, it has triggered a shift of
  ...
  systematically refined\TB{, always keeping}
% . More importantly, these simplified analyses still make
    explicit the methodological commitments and the assumed prior
  ...
  from the underlying assumptions. \TB{These methods, though, 
    often appear as a `black box' to traditional practitioners.
    In fact, the switch to a statistical view
    complicates comparison of the
    results from these newer methods with traditional understanding,
    sometimes leading to misinterpretation and overly broad claims.} 
  We describe here this evolving
  methodological shift, attributed to the advent of big\TB{, but often
    incomplete and poorly curated,} data, \TB{emphasizing the
    underlying similarity of the newer quantitative to the
    traditional comparative methods and discussing when and to what
    extent the former have advantages over the latter. In this review,
    we cover briefly both} randomization tests for detecting patterns in a largely
...
%  genes that
  \TB{core} components of indispensable subsystems can be transmitted
...
to cognates.\footnote{\TB{We note that the unrelated words are slightly more \emph{dissimilar} than one would expect random pairs to be.  We do not study this further here, but it would be expected if, on average, distinct words in a language `repelled' each other phonetically.}} The right panel shows analogous data for the Guaycuruan
...
still visible. \NR{\TB{The difference is not necessarily significant---the exact distributions depend strongly} on the amount of training data \TB{available.}
% and especially the amount of trained cognates results in this language family specific 
% peak structure, i.e. it does not look exactly the same for all families.
}
...
traditional historical linguists, \TB{their real value is in
  situations where the evidence is scarce or contradictory.  In such
  cases, the results of the analysis depend on prior beliefs and
  assumptions about consistancy with global patterns.  Whereas a
  trained linguist can see that the evidence is weak, the quantitative
  method is often better able to integrate such weak evidence from
  various pieces of data, keeping track of the accumulated weight of
  evidence. Thus, for example, while the most widely accepted opinion
  is that}
% they constitute the only alternative
...
%
%
% By using a combination of methods from evolutionary biology
% ---prominently, phylogenetic inference---
Bowern \TB{\cite{Bowern:12a}} was able to \TB{use quantitative methods
  to} gather enough evidence for 4\TB{--}5 groupings 
% (tentatively referred as macro-families) present in the wordlists
...
not all directly related
%, at least to
\TB{at} the time depth accessible to \TB{available} methods and the data.
% Not only did the usage of methods from evolutionary
...
\NR{Nakh-Daghestanian} languages \NR{(mostly form the Daghestanian branch, 
Nakh languages marked with asterix)}, as well as for a sample of 155 languages from the
IDS \NR{(see Table~\ref{tab:langDeltas})}. If similar, non-identical words do not systematically have similar
...
\end{center}
\caption{Distribution of $\delta$ in Indo-European, \NR{ Nakh-Daghestanian}, and

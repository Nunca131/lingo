...
An important issue to consider in the choice of traits to analyze is
  ...
  sound-meaning correspondences \cite{Blasi:2016} or high-level typological 
  variables (such as word order and the presence of certain marking strategies, \cite{Dunn:05}) might be more
  ...
  method is infallible \NEW{with respect to the features chosen}. If
  \NEW{these} are either too noisy to convey retrievable information or are
  not \NEW{transmitted along the same tree} to begin with, the \NEW{reconstructed} tree
  might not depict \NEW{any} actual relationships. An extreme example in the
  phylogeny of animals is the strange case of \emph{Xenoturbella},
  \NEW{which was placed at vastly different positions in the animal tree
  ...
  feature sets \NEW{because it is often not obvious \textit{a priori} which
...
\NEW{incomplete} lineage sorting \NEW{\footnote{Incomplete lineage sorting
...
from tree-likeness in the data. Lineages are not always well-separated, as in the
...
the presence of other forms of \NEW{relationships}, trees are a useful
...
analysis used only the core concepts \PFS{included in work lists} \TODO{add
  a few references to word lists here?}
...
often reproduced known events in linguistic history \TODO{can we simply
  expand the following list by a few more references?}
\cite{Lee:11,Gray:09,Dunn:11}, and provided a credible starting point in
discussions on language relationships \cite{Bouckaert:12}.  It is important
to realize, though, that all these approaches crucially depend on manually
determined cognate classes. There is not yet an equally reliable automatic
method to determine cognacy, although contemporary automatic methods
achieve reasonably good rates of classification \cite{List:2017}. Any
progress in the development of such methods has to rely on a good inference
of sound correspondences, which seems to require larger datasets.

\par\framebox{\begin{minipage}{0.9\textwidth}\small
\textbf{Discussion of comment: delete when resolved!}

\TODO{Regarding rewrites in the paragraph above:}


\DB{swadesh is just one of such lists - Jakarta-Leipzig is way more common
  these days, for instance. I'd say axe this reference to Swadesh.}
\TODO{OK deleted} 
...
\DB{it is fair to comment here on the automatic cognate
  detection algorithms, some of which achieve efficiency comparable to that
  of humans - I added Mattis' most recent paper}.
\end{minipage}}
...
\par\framebox{\begin{minipage}{0.9\textwidth}\small 
    ...
    \TODO{DONE These studies do not "prove genealogical persistence", they
      are circular in this respect since they assume families established
      on the assumption of genealogical persistence of the very same
      vocabulary items... TANMOY} \DB{I agree - however, do we really need
      this whole section on IDS? I don't think so IMHO} \TODO{Since
      previous reviewers insisted on the IDS detail, we should keep this
      discussion here.}
\end{minipage}}
...
\TODO{DONE ABOUT WRONG ALIGNMENTS.
  THE CASE WHERE THE COGNATES ARE NOT KNOWN IS NOT YET TESTED EXCEPT SOME
  WORK WITH HRUSCHKA. What evidence supports this claim that there is
  sufficient signal in real cognate pair alignments for correct sound
  correspondences to be identified in spite of some false positives/wrong
  alignments?  TANMOY} \DB{I agree on that the claim comes across as a bit
  vague.}

\PFS{Given Tanmoy's reply, I suggest that we simply delete the following 
sentence:
\begin{small} 
  Even if a fraction of the candidate cognate pairs are false positives and
  some of the alignments of cognate pairs are wrong as a consequence of the
  oversimplified scoring model, there is enough signal in the correct
  alignments of real cognate pairs for the statistical machinery to
  identify the more common sound correspondences.
\end{small}}
\end{minipage}}
...
\TODO{The reader may be left wondering about situations where one
  ...
  being more explicit about what "gaps" are in this method. NANCY} \NR{For
  this todo maybe point to Fig.~\ref{fig:aln}? maybe I can find another
...
\NEW{Even though cognacy itself is a binary property, keeping track of
...
should weight the stable words and \NEW{sounds (e.g., consonants in
  most language families)} more. 
\TODO{DONE Is this a language independent result? DAMIAN}
The signal

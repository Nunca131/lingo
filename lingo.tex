...
An important issue to consider in the choice of traits to analyze is
  ...
  One of the key take-home lessons from phylogenetic studies in both
  biology and linguistics is that a detailed understanding of the
  mechanisms of change is helpful, but \TT{not} a necessary prerequisite,
  for the reconstruction of phylogenetic relationships. The causal
  structure imposed by vertical inheritance on a branching \TT{tree}
  dictates that similarity of randomly originating features follows a
  hierarchically nested structure consistent with the partitions of the
  tree. If the feature set under study happens to contain enough
  independently evolving, vertically inherited neutral traits, their
  similarity alone can be used to reconstruct the phylogeny. While detailed
  stochastic models of mutational change have become the method of choice
  at the level of DNA and protein sequences, perfectly valid phylogenies
  can be obtained by investigating complex phenotypic characters such as
  specific shapes of bones, the presence or absence of particular
  neurotransmitters, or the spatial organization of neuronal networks. In
  fact, when there is no mechanistic understanding of the patterns of
  change, robust, non-parametric approaches to tree reconstruction are
  often the method of choice\NEW{~\cite{Waegele:DMP, Dunn:15}}. Of course no
  method is infallible \NEW{with \TB{respect} to the features chosen}. If
  \NEW{these} are either too noisy to convey retrievable information or are
  not \TB{transmitted along the same tree} to begin with, the \TB{reconstructed} tree
  might not depict \TB{any} actual relationships. An extreme example in the
  phylogeny of animals is the strange case of \emph{Xenoturbella},
  \PFS{which was placed at vastly different positions in the animal tree
    based on different choices of data sets and taxa included for comparion
    \cite{Bourlat:06,Rouse:16}.}  This underlines the fundamental
  importance of basing phylogenetic reconstruction on multiple, independent
  feature sets \PFS{because it is often not obvious \textit{a priori} which
...
\DB{The CCA-adding enzymes example might be a tad too technical for the intended audience -
 what about reducing the Xentourbella example to one or two lines?}
\\
...
\TB{I prefere the CCA-adding enzyme example, primarily because I think moving away from the
Xenoturbella example will be easier to argue---give the referee a win---but reduced to 1-2 lines
like the current Xenoturbella discussion.}
\\
DELETED:
...
known in the biological phylogenetic literature. \TB{This graded
  ...
  parameterize the entire process.} \TODO{DONE The logic here is
  unclear. Cf. the comment above that we never actually told how only using
  tree-models tells us whether they are appropriate or not. But suppose we
  did, then the authors suggest we can systematically improve the
  tree-model by extending it to become ARGs. Then why not use ARGs to begin
  with? (That would be systematic, rather than ad hoc). TANMOY} \DB{I feel
  the whole discussion on the tree/non-tree like tension is too long
  already...}
...
lists \DB{swadesh is just one of such lists - Jakarta-Leipzig is way more 
common these days, for instance. I'd say axe this reference to Swadesh.}
---assumed to be the most closely co-inherited roots, refractory to
horizontal transfer---or, at least, weighted their evidence more, this 
often reproduced known events in linguistic history~\cite{Lee:11,Gray:09,Dunn:11}
\TODO{DONE What is meant by "very good fit"? The whole paper argues we should
...
this metholodogy). TANMOY}\DB{agree with removing Dunn et al's ref here. As 
for the claim: I propose we replace "very good fit" for "qualitatively 
agrees most of the time with" although, as a matter of fact, most of the 
fit is simply topological in nature (e.g. a particular branch is 
recovered, or two close variates show up as close sisters in a tree.)}
...
The core or basic vocabulary when compiled
into such word lists is particularly suited for the phylogenetic enterprise
for two reasons. First, it is composed of many items of \NEW{plausible} genealogical
...
\TODO{DONE These studies do not "prove genealogical persistence", they are circular in
this respect since they assume families established on the assumption of
genealogical persistence of the very same vocabulary items... TANMOY} 
\DB{I agree - however, do we really need this whole section on IDS? I don't think so IMHO}
...
cognates preferentially match corresponding sounds with each other.
\TB{One can then introduce gaps in the alignment to improve thr match
  according to this draft correspondence table. Iteration of this
  process of improving the correspondence table followed by improving
  the alignment rapidly reduces the number of unexplained changes and
  refines the correspondence table~\cite{Hruschka:14}.} The
...
\TODO{DONE ABOUT WRONG ALIGNMENTS.  THE CASE WHERE THE COGNATES ARE
  NOT KNOWN IS NOT YET TESTED EXCEPT SOME WORK WITH HRUSCHKA. What
  evidence supports this claim that there is sufficient signal  
in real cognate pair alignments for correct sound
correspondences to be identified in spite of some false positives/wrong alignments? 
TANMOY} \DB{I agree on that the claim comes across as a bit vague.}
...
\TODO{The reader may be left wondering about situations where one
  sound/symbol in language A corresponds to two in language B (e.g. Spanish
  puerta - Italian porta, or instances where two symbols correspond to a
  single sound in one language). This is one of the reasons that I
  recommend consolidating the discussion of representational issues and
  being more explicit about what "gaps" are in this method. NANCY}  
  \NR{For this
  ...
  obviously is one.}\LM{Would be cool if we could use the examples above,
  e.g. bueno and bono. Do we have an fitting scoring model for this?}

\PFS{I think this is really taken core of in the Figure the new text.}
...
Based on this, one can proceed to define the total score \NEW{$\sigma(u,v)$
  ...
  involving gaps, \ie{} insertions or deletions
  of characters. We can write this in form 
  \begin{equation} 
    \sigma_{AB}(u,v) = 
...
alignments of words from 11 Indo-European languages \NEW{(Table~\ref{tab:bigramscores})}.
For both scoring
...
sound along with word boundaries\LM{. However, neighboring sounds are} an imperfect proxy for syllable
boundary. 
\TODO{Word boundaries aren't really an 'imperfect proxy' for syllable 
boundaries. Rather, they are a restricted subset of syllable boundaries
that are recoverable from orthographic representations.\LM{seriously???}}
In more difficult cases, such as the Guaycuruan example, the
...
\TB{Even though cognacy itself is a binary property, keeping track of
  the weight of evidence that establishes a set as cognate is
  essential to any quantitative further analyses based on cognate
  sets. Proper procedures for propagating uncertainties from one stage
  of the calculation to the next is a key strength of formal methods
  we advocate.}
\TODO{DONE But cognacy is a yes/no property and modeled as such in the work
the authors cite, so the inability to provide a yes/no answer is
incompleteness rather than a virtue. TANMOY}
Any further
...
for each choice of the cutoff. We will return to this point again several
...
time depth, and words had already been categorized into probable
...
\TODO{DONE SEE END OF PARA It's more customary to present the f-score
  of the precision/recall corresponding to the different threshold
  values... TANMOY  
\LM{replaced 'It is customary to' with 'One can'}}
...
$0.9$. \TB{Since the ROC curves show
...
focus on alignments and their score\NEW{s} conforms more closely to the paradigm
of sequence comparison in computational biology, while the comparison of
probabilistic models focuses on the evolutionary processes and is somewhat
closer to the traditional conceptualization in linguistics. 
\TODO{do you mean the "alternative" DAMIAN}
Nevertheless,
the two methods are largely equivalent and can answer essentially the same
questions, albeit using different mathematical and computational
techniques.
\TODO{These methods aren't equivalent from a traditional historical 
linguist's perspective, and probably wouldn't deserve any
lengthy discussion as alternatives if they were truly equivalent. 
Perhaps a different word choice or a modifier on "equivalent" would make 
more sense here. DAMIAN}
...
that are compared. \TB{At the same time, neither method makes a
...
language pairs or language families.  \TB{Again, such data-driven
...
\TODO{DONE Later in the paper sound change processes are argued be
  universal (a la Ohala) which means that there is a specific answer
  to the incertitude expressed in the above passage. TANMOY}
\TODO{DONE The "regular correspondences" you refer to here are the same as the 
...
as in the Neogrammarian usage. TANMOY}
...
feasible at least from a computational point of view, \PFS{as examplified
...
proto-languages and then the regularities of sound change.  \TODO{DONE
  This is
  an important question, but unfortunately not prudently discussed in this
  passage. Since the components depend on each other, it is not
  probabilistically correct to do it in the iterative manner, and, as
  argued elsewhere in the paper (p 16) it is not a correct approach from
  the perspective of propagating uncertainty. TANMOY} This separation of
tasks is \TB{DELETE WORD: merely} a matter of computational efficiency
and expediency \TB{at our current computational capability.} In
...
computed for a given tree, and then used to compare trees. \TB{The
  question is actually of propagating the alignment uncertainty into
  phylogenetic reconstruction.} Experience from
computational biology \TB{DELETE:, furthermore,} shows that
\TB{DELETE: it is
often pragmatically favorable to perform as much of the analysis as
possible on directly observable data.} \NEW{\TB{DELETE: For example,}
  \TB{though} methods that attempt to concurrently estimate alignments and
  phylogenetic trees \cite{Lunter:05,Heman:14} \TB{can be}
  devised and applied with success, it appears \TB{to} gain
  \TB{little} in accuracy over the common practise \TB{DELETE: (} of
  treating alignment and phylogeny reconstructing as independent tasks
  \TB{DELETE: ) is very moderate} and often does not
...
of computational methodology. For example, reliable reconstruction of the
subgrouping within a language family or identification of macro-families
can be conducted on the basis of presence and absence of cognates without
the necessity to model sound changes explicitly
\cite{Bowern:12a,Bowern:12b}.
\TODO{How do you know the subgrouping in Bowern, 2012; Bowern and
...
\QUOTE{random} mutations \TB{(either sporadic changes or
...
\TODO{DONE It is not entirely clear what this sentence means, or how to evaluate 
...
discussion would be very helpful. TANMOY}
...
at an early stage. Thus, for example, typological errors often introduce
...
extremely rare occurences of characters or sequence of characters can then
be flagged for manual verification.
\TODO{Do you mean "typographical errors"? As written, the meaning of this 
sentence is unclear. DAMIAN}
...
\TODO{DONE THIS NEEDS ANALYSIS BEYOND THE SCOPE OF THE PAPER. THE IDEA
  IS TO USE MEANING SIMILARITIES \emph{DISCUSSED IN 4.1-2} TO DO THE
  PERMUTATION TEST. This idea is limited by regularities in the meaning space of the kind
...
assessment of number emanating from a permutation test. TANMOY: do you 
know know what could be meant here?}
...
many non-alphabetic writing systems do not directly encode the phonetics at
all.
\TODO{Do alphabetic scripts do that? Or is segmental phonology rather than
"phonetics" intended? DAMIAN}
...
we found a residual level of about 3\% discrepancies, 
\TODO{DONE This conclusion should be qualified by the fact that only closely related
...
small amount of cognates. DAMIAN: is there a good reference (maybe by List)? 
TANMOY: example with Turkic languages appropriate here?}
...
cognate call is independent of the encoding scheme. \TB{It is to be
...
should weight the stable words and \TB{sounds (e.g., consonants in
  most language families)} more. 
\TODO{DONE Is this a language independent result? DAMIAN TANMOY}
The signal
...
native population experienced..." DAMIAN}
...
molecular clock hypothesis in molecular evolution. Rate constancy has been
...
context of a likelihood based approach, the relative constancy of the rates
...
\par\framebox{\begin{minipage}{0.9\textwidth}\small
\textbf{Discussion of comment: delete when resolved!}
\TODO{DONE AGREED The conclusion contains information not in the body of the paper and
should thus be reorganized. TANMOY DAMIAN PETER NANCY - new section heading 
``Conclusion and ...''}
\end{minipage}}
...
\TB{explicitly} incorporate \TB{the regularities in} rare phonetic
processes like metathesis and \TB{other irregular processes like}
analogical change \TB{(treating them, rather, as instances of independent
sporadic changes)}, 
...
\TODO{DONE Though analogical change is sometimes treated as a "garbage 
...
poorly handled by these methods. TANMOY}
The
